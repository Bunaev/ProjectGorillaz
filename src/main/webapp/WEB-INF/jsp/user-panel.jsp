        ...
        <c:if test="${sessionScope.user!=null}">
            <fieldset class="userPage" style="margin-top: 5%">
                ...
                <form class="form-horizontal" method="post">
                    ...
                    <input id="name" name="name" type="text" value="${sessionScope.user.username}" class="input">
                    </br>
                    <label class="legend" style="display: block;float: left;width: 80px;height: 15px;" for="login">Логин: </label>
                    <input id="login" name="login" type="text" value="${sessionScope.user.login}" class="input">
                    </br>
                    <label class="legend" style="display: block;float: left;width: 80px;height: 15px;" for="password">Пароль: </label>
                    <input id="password" name="password" type="password" value="${sessionScope.user.password}" class="input">
                    </br>
                    <button formmethod="post" id="save" name="saveChangeUser" value="save" class="btn btn-success">Сохранить</button>
                </form>
            </fieldset>
        </c:if>
        <c:if test="${sessionScope.games!=null}">
            <strong class="legend" style="margin-top: 2%;margin-left: 2%;font-weight: 600;">Всего сыграно: ${sessionScope.gameCount} игр. Из них: ${sessionScope.loose} поражений и ${sessionScope.winner} побед.</strong>
            <div style="display: inline-table; margin-top: 2%">
                <c:forEach var="game" items="${sessionScope.games}">
                    ...
                    <table class="table">
                        ...
                        <tr>
                            <td>Прогресс:</td>
                            <td>Вы ответили на ${game.progress} вопросов из ${sessionScope.countPages}.</td>
                        </tr>
                        </tbody>
                    </table>
                    </c:if>
